...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    async logout(isAutomated = false) {
      // Tambahkan parameter isAutomated
      this.error = null
      try {
        const chatStore = useChatStore()
        // Hanya panggil API logout jika bukan logout otomatis dari interceptor
        if (!isAutomated) {
          await authService.logout()
          chatStore.clearSession()
        } else {
          // Jika otomatis, cukup bersihkan sesi chat lokal
          chatStore.clearSession()
        }
      } catch (error) {
        // Logging error ini hanya jika ini adalah logout manual yang gagal API
        // Atau jika automated logout memiliki masalah lain selain 401
        if (!isAutomated) {
          console.error('Logout API call failed, but user is logged out locally:', error)
        } else {
          // Untuk automated logout, error 401 sudah biasa, jadi tidak perlu panik
          console.log('Automated logout due to 401. Local data cleared.')
        }
      } finally {
        this._clearAuthData() // Selalu bersihkan data lokal
        this.loginRedirectPath = null
        this.redirectOnClosePath = null
        this.closeLoginModal()
        this.status = 'idle'

        // Logika redirect setelah logout
        // Redirect ke halaman Home jika sedang berada di rute yang memerlukan otentikasi
        // atau jika bukan di halaman Home.
        if (
        ...
        // Jika sudah di Home (dan Home tidak memerlukan auth), tidak perlu redirect lagi
      }
    },
